...
double Graph::CalculateForPoints(PointGraph* parent)
{
    ...
    for (unsigned int i = 0; i < points->size(); i++)
    {
        ...
        if (res < 0)
        {
            ...
            {
                if (i >= 2 && res == -2 && (*points)[i - 1]->x - (*points)[i - 2]->x >= 2)
                {
                    // если между предыдущии двумя точками есть промежуток >= 2 , то перемещаем точку туда
                    (*points)[i]->x = (*points)[i - 2]->x + 1;
                }
                else
        ...
        // увеличиваем координату своего уровня, если она была меньше текущей точки
        if(levelsX[(*points)[i]->level] < (*points)[i]->x + 1)
